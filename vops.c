...
Datum
vops_bool_input(PG_FUNCTION_ARGS)
{
	char const *str = PG_GETARG_CSTRING(0);
	vops_bool  *result;
...
Datum
vops_bool_output(PG_FUNCTION_ARGS)
{
...
Datum
vops_bool_not(PG_FUNCTION_ARGS)
{
...
Datum
vops_count_accumulate(PG_FUNCTION_ARGS)
{
...
Datum
vops_count_any_accumulate(PG_FUNCTION_ARGS)
{
	vops_tile_hdr *opd = (vops_tile_hdr *) PG_GETARG_POINTER(1);
	int64		count = PG_GETARG_INT64(0);
...
Datum
vops_count_any_extend(PG_FUNCTION_ARGS)
{
...
Datum
vops_count_extend(PG_FUNCTION_ARGS)
{
	vops_int8  *state = (vops_int8 *) PG_GETARG_POINTER(0);
	int64		count = state->payload[TILE_SIZE - 1];
...
Datum
vops_count_all_accumulate(PG_FUNCTION_ARGS)
{
	int64		count = PG_GETARG_INT64(0);
	int			i;
...
Datum
vops_time_bucket(PG_FUNCTION_ARGS)
{
...
Datum
vops_date_bucket(PG_FUNCTION_ARGS)
{
	Interval   *interval = PG_GETARG_INTERVAL_P(0);
...
static void
insert_tuple(Datum *values, bool *nulls)
{
...
Datum
vops_avg_final(PG_FUNCTION_ARGS)
{
...
Datum
vops_avg_combine(PG_FUNCTION_ARGS)
{
...
Datum
vops_avg_serial(PG_FUNCTION_ARGS)
{
	vops_avg_state *state = (vops_avg_state *) PG_GETARG_POINTER(0);
	StringInfoData buf;
	bytea	   *result;
...
Datum
vops_avg_deserial(PG_FUNCTION_ARGS)
{
...
Datum
vops_wavg_final(PG_FUNCTION_ARGS)
{
...
Datum
vops_var_samp_final(PG_FUNCTION_ARGS)
{
	vops_var_state *state = (vops_var_state *) PG_GETARG_POINTER(0);
...
Datum
vops_var_pop_final(PG_FUNCTION_ARGS)
{
	vops_var_state *state = (vops_var_state *) PG_GETARG_POINTER(0);
...
Datum
vops_stddev_samp_final(PG_FUNCTION_ARGS)
{
	vops_var_state *state = (vops_var_state *) PG_GETARG_POINTER(0);
	double		var;
...
Datum
vops_stddev_pop_final(PG_FUNCTION_ARGS)
{
	vops_var_state *state = (vops_var_state *) PG_GETARG_POINTER(0);
...
Datum
vops_var_combine(PG_FUNCTION_ARGS)
{
...
Datum
vops_var_serial(PG_FUNCTION_ARGS)
{
	vops_var_state *state = (vops_var_state *) PG_GETARG_POINTER(0);
	StringInfoData buf;
	bytea	   *result;
...
Datum
vops_var_deserial(PG_FUNCTION_ARGS)
{
	bytea	   *sstate = PG_GETARG_BYTEA_P(0);
...
Datum
vops_agg_serial(PG_FUNCTION_ARGS)
{
...
Datum
vops_agg_deserial(PG_FUNCTION_ARGS)
{
	bytea	   *sstate = PG_GETARG_BYTEA_P(0);
...
inline static void
merge_zero_bits(uint8 *dst_max_zero_bits, uint8 *src_max_zero_bits)
{
	int			i;
...
Datum
vops_approxdc_final(PG_FUNCTION_ARGS)
{
...
Datum
vops_approxdc_combine(PG_FUNCTION_ARGS)
{
...
Datum
vops_approxdc_serial(PG_FUNCTION_ARGS)
{
	vops_approxdc_state *state = (vops_approxdc_state *) PG_GETARG_POINTER(0);
...
Datum
vops_approxdc_deserial(PG_FUNCTION_ARGS)
{
...
Datum
vops_text_approxdc_accumulate(PG_FUNCTION_ARGS)
{
...
Datum
vops_deltatime_output(PG_FUNCTION_ARGS)
{
	Interval	interval;
...
Datum
vops_deltatime_input(PG_FUNCTION_ARGS)
{
	Interval   *interval =
...
Datum
vops_time_interval(PG_FUNCTION_ARGS)
{
	Interval   *interval = PG_GETARG_INTERVAL_P(0);
...
Datum
vops_text_input(PG_FUNCTION_ARGS)
{
	char const *str = PG_GETARG_CSTRING(0);
...
Datum
vops_text_output(PG_FUNCTION_ARGS)
{
...
Datum
vops_text_typmod_in(PG_FUNCTION_ARGS)
{
...
Datum
vops_text_const(PG_FUNCTION_ARGS)
{
	text	   *t = PG_GETARG_TEXT_P(0);
	char	   *const_data = (char *) VARDATA(t);
	size_t		const_size = VARSIZE(t) - VARHDRSZ;
...
Datum
vops_text_concat(PG_FUNCTION_ARGS)
{
	struct varlena *vl = PG_GETARG_VARLENA_PP(0);
	struct varlena *vr = PG_GETARG_VARLENA_PP(1);
	vops_tile_hdr *left = VOPS_TEXT_TILE(vl);
...
Datum
vops_betwixt_text(PG_FUNCTION_ARGS)
{
	struct varlena *var = PG_GETARG_VARLENA_PP(0);
...
Datum
vops_ifnull_text(PG_FUNCTION_ARGS)
{
	struct varlena *var = PG_GETARG_VARLENA_PP(0);
	vops_tile_hdr *opd = VOPS_TEXT_TILE(var);
	size_t		elem_size = VOPS_ELEM_SIZE(var);
...
Datum
vops_coalesce_text(PG_FUNCTION_ARGS)
{
...
Datum
vops_text_first(PG_FUNCTION_ARGS)
{
	struct varlena *var = PG_GETARG_VARLENA_PP(0);
	vops_tile_hdr *tile = VOPS_TEXT_TILE(var);
	size_t		elem_size = VOPS_ELEM_SIZE(var);
...
Datum
vops_text_last(PG_FUNCTION_ARGS)
{
	struct varlena *var = PG_GETARG_VARLENA_PP(0);
	vops_tile_hdr *tile = VOPS_TEXT_TILE(var);
	size_t		elem_size = VOPS_ELEM_SIZE(var);
...
Datum
vops_text_low(PG_FUNCTION_ARGS)
{
	struct varlena *var = PG_GETARG_VARLENA_PP(0);
	vops_tile_hdr *tile = VOPS_TEXT_TILE(var);
	size_t		elem_size = VOPS_ELEM_SIZE(var);
...
Datum
vops_text_high(PG_FUNCTION_ARGS)
{
	struct varlena *var = PG_GETARG_VARLENA_PP(0);
	vops_tile_hdr *tile = VOPS_TEXT_TILE(var);
	size_t		elem_size = VOPS_ELEM_SIZE(var);
...
Datum
vops_text_first_accumulate(PG_FUNCTION_ARGS)
{
	vops_first_state *state = (vops_first_state *) PG_GETARG_POINTER(0);
...
Datum
vops_text_last_accumulate(PG_FUNCTION_ARGS)
{
	vops_first_state *state = (vops_first_state *) PG_GETARG_POINTER(0);
	struct varlena *vars = PG_GETARG_VARLENA_PP(1);
	vops_int8  *tss = (vops_int8 *) PG_GETARG_POINTER(2);
...
Datum
vops_first_final(PG_FUNCTION_ARGS)
{
	vops_first_state *state = (vops_first_state *) PG_GETARG_POINTER(0);
...
Datum
vops_first_combine(PG_FUNCTION_ARGS)
{
...
Datum
vops_last_combine(PG_FUNCTION_ARGS)
{
	vops_first_state *state0 = (vops_first_state *) PG_GETARG_POINTER(0);
	vops_first_state *state1 = (vops_first_state *) PG_GETARG_POINTER(1);
...
Datum
vops_populate(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < n_attrs; i++)
	{
		...
		if (types[i].tid != VOPS_LAST && types[i].len < 0)
		{						/* varying length type: extract size from
								 * atttypmod */
...
static void
vops_import_error_callback(void *arg)
{
...
Datum
vops_win_final(PG_FUNCTION_ARGS)
{
	filter_mask = ~0;			/* it is assumed that at the moment of window
								 * aggregate finalization, filter_mask is not
								 * needed any more, so reset it */
	PG_RETURN_POINTER(PG_GETARG_POINTER(0));
}


static vops_agg_state *
vops_create_agg_state(int n_aggregates)
{
	HASHCTL		ctl;
	vops_agg_state *state;
...
static vops_agg_state *
vops_init_agg_state(char const *aggregates, Oid elem_type, int n_aggregates)
{
	vops_agg_state *state;
	int			i,
				j;
...
static void
vops_agg_state_accumulate(vops_agg_state *state, int64 group_by, int i, Datum *tiles, bool *nulls)
{
	int			j;
	bool		found;
	vops_group_by_entry *entry = (vops_group_by_entry *) hash_search(state->htab, &group_by, HASH_ENTER, &found);
...
Datum
vops_agg_final(PG_FUNCTION_ARGS)
{
...
Datum
vops_agg_combine(PG_FUNCTION_ARGS)
{
	int			i;
...
Datum
vops_char_concat(PG_FUNCTION_ARGS)
{
...
Datum
vops_int2_concat(PG_FUNCTION_ARGS)
{
...
Datum
vops_int4_concat(PG_FUNCTION_ARGS)
{
...
Datum
vops_is_not_null(PG_FUNCTION_ARGS)
{
	vops_bool  *result = (vops_bool *) palloc(sizeof(vops_bool));
...
Datum
vops_window_reduce(PG_FUNCTION_ARGS)
{
...
Datum
vops_lag_reduce(PG_FUNCTION_ARGS)
{
	if (PG_ARGISNULL(0))
	{
		PG_RETURN_NULL();
...
Datum
vops_initialize(PG_FUNCTION_ARGS)
{
	PG_RETURN_VOID();
}
...
is_select_from_vops_projection(Query *query, vops_var *var)
{
	...
	for (relno = 1; relno <= n_rels; relno++)
	{
		...
		{
			Relation	rel = heap_open(rte->relid, NoLock);	/* Assume we already
																 * have adequate lock */
			int			attno;
...
vops_expression_tree_mutator(Node *node, void *context)
{
	...
	if (IsA(node, BoolExpr))
	{
		...
		if (expr->boolop == NOT_EXPR)
		{
			if (list_length(vector_args) != 0)
			{
				...
				/*
				 * Transform expression (NOT filter(o1)) to
				 * (filter(vops_not(o1))
				 */
		...
		{
			/*
			 * Transaform expression (filter(o1) AND filter(o2)) to
			 * (filter(vops_and(o1, o2)))
			 */
	...
	else if (IsA(node, CoalesceExpr))
	{
		...
		if (tid != VOPS_LAST && list_length(coalesce->args) == 2)
		{						/* TODO: right now only two arguments case is
								 * handled */
...
typedef struct
{
	...
	int			n_rels;			/* number of relations in query (join
								 * pseudorelations) */
	Const	  **consts;			/* array references to Const nodes in query,
								 * indexed by literal location */
	Bitmapset  *operands;		/* columns used in one expression */
	vops_table_refs *refs;		/* usage of table' variables (index by
								 * relation number - 1) */
...
vops_substitute_tables_with_projections(char const *queryString, Query *query)
{
	...
	Bitmapset  *vectorCols = NULL;	/* bitmap set of vector columns from all
									 * tables: (attno*n_rels + relno - 1) */
	Bitmapset  *scalarCols = NULL;	/* bitmap set of scalar columns from all
									 * tables: (attno*n_rels + relno - 1) */
	...
	for (relno = 0, fromno = 0; relno < n_rels; relno++)
	{
		...
		for (i = 0; i < SPI_processed; i++)
		{
			...
			if (bms_is_subset(refs->where, allAttrs)	/* attributes in WHERE
														 * clause can be either
														 * scalar either vector */
				&& bms_is_subset(refs->agg, vectorAttrs)	/* aggregates are
															 * calculated only for
															 * vector columns */
				&& bms_is_subset(refs->other, scalarAttrs)) /* variables used in
															 * other clauses can be
															 * only scalar */
			{
				...
				/*
				 * Update vector/scalar bitmap sets for this query for this
				 * projection
				 */
				for (j = 0; j < nVectorColumns; j++)
				{
	...
	if (select != NULL			/* We did some substitution... */
		&& hasAggregates		/* use projection only if we do some
								 * aggregation */
		&& vops_check_clusters(pullvar_ctx.clusters, vectorCols, scalarCols))	/* vector and scalar
																				 * columns are not mixed
																				 * in one expression */
	{
...
#define parse_analyze parse_analyze_fixedparams
			...
			*query = *subst;	/* Urgh! Copying structs is not the best way
								 * to substitute the query */
		}
		PG_CATCH();
		{
			...
			/*
			 * We are leaking some relcache entries in case of error, but it
			 * is better then aborting the query
			 */
			FlushErrorState();
		}
		PG_END_TRY();
...
vops_resolve_functions(void)
{
	...
	if (is_not_null_oid == InvalidOid)
	{
		...
		is_not_null_oid = LookupFuncName(list_make1(makeString("is_not_null")), 1, &any, true); /* lookup last functions
																								 * defined in extension */
		if (is_not_null_oid != InvalidOid)	/* if extension is already
											 * intialized */
		{
...
#if PG_VERSION_NUM>=140000
static void
vops_post_parse_analysis_hook(ParseState *pstate, Query *query, JumbleState *jstate)
#else
static void
vops_post_parse_analysis_hook(ParseState *pstate, Query *query)
#endif
{
	vops_var	var;
	...
	if (query->commandType == CMD_SELECT &&
		...
		pstate->p_paramref_hook == NULL)	/* do not support prepared
											 * statements yet */
	{
				  ...
				  ParamListInfo params)
{
		...
		params == NULL)			/* do not support prepared statements yet */
	{
		...
		memset(explain, ' ', prefix);	/* clear "explain" prefix: we need to
										 * preseve node locations */
